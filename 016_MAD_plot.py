...
dict={'source_id': source_id_new, 'ra': ra_new, 'ra_error': ra_error_new, 'dec': dec_new, 'dec_error': dec_error_new, 'angDist': dist_new, 'pmra': pmra_new, 'pmra_error': pmra_error_new, 'pmde': pmde_new, 'pmde_error': pmde_error_new,'b_rgeo': brgeo_new, "B_rgeo": Brgeo_new, 'theta': theta}
df1 = pd.DataFrame(dict) 
    
# saving the dataframe 
df1.to_csv('New_SFO38_theta.csv')

for (i,j,k,l) in zip(ra_new, dec_new, pmra_new, pmde_new):
	X = i
	Y = j
	...
	# Creating plot
	quiver(X, Y, U, V, color='b', units='xy', scale=1)
	
xlim(324.8, 325.9)
ylim(58.10, 58.55)
xlabel('RA')
ylabel("Dec")
savefig('ra_dec_vect.png')
